...
class Permissions:
    ...
    def __init__(self, permissions_filename):
        ...
        logfile = LogFile(
            ...
            parent_prefix = "permissions"
        )
        logfile.log(f"permissions system loaded and ready.")
    def load(self):
        """
        ...
        if command in self.permissions:
            ...
            for chann in channels:
                try:
                    ...
                    logfile.log(f"permissions config warning: {command} allowed in {chann} but {chann} cant be found.")
                    continue
                if (channel_id == chann_id):
                    return True
        return False
    def check(self, command_name, ctx):
        """
        alias for is_allowed that also logs the attempt.
        """
        allowed = self.is_allowed(command_name, ctx.channel.id)
        if allowed:
            msg = f"accepted_command:"
        else:
            msg = f"rejected_command:"
        msg += f"{ctx.channel.name}:{ctx.author.name}:{ctx.message.content}"
        logfile.log(msg)
        return allowed
