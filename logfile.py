...
class LogFile:
    ...
    """
    def __init__(self, filename, parent=None, parent_prefix="", prefix = ""):
        """
        ...
        """
        # option to create new logfiles each time - disabled for now
        # more practical to just have in one file, TODO: only when file too big
    ...
    def log(self, text, prefix=""):
        ...
        Appends text to this logfile, with "datetime : " prepended to it.
            prefix - overwrites the logfiles current prefix if set
        """
        if prefix != "":
            text = f"{prefix} : {text}"
        else:
