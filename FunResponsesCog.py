...
async def on_message_funresponse(message, funresponsecog):
    ...
    if "zbot" in text:
        ...
        if (
            ...
            return   
    if "!skillissue" in text:
           await channel.send("https://cdn.discordapp.com/attachments/1036778189978079362/1058424261192192092/unknown.png")
           return   
    if "ualac" in text:
        val = random.randrange(0,10)
        if val <= 2:
            await channel.send("<:deflectualac:961374856367177728>")
            return
    if "pjota" in text:
        val = random.randrange(0,9)
        if val <= 2:
            await channel.send("<:deflectjota:948622963480068176>")
            return
    if "jota" in text:
        val = random.randrange(0,9)
        if val <= 2:
    ...
    if "bat" in text:
        val = random.randrange(0,10)
        if val == 0:
            await channel.send("<:deflectbat:1047573829091864707>")
            return
    if "wuhan" in text:
        val = random.randrange(0,10)
        if val == 0:
