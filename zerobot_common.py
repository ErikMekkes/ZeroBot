...
Most things in here should be referenced directly and fully: import 
zerobot_common and access as zerobot_common.something. Do not use imports like
`from zerobot_common import x`, and do not make local copies for easy reference.
Because if you do, you have to be EXTREMELY careful they are correctly updated. 
Example why: if you use in a different file : _easy_name = zerobot_common.variable1, that copy will point 
to the same object initially, but if variable1 in here is ever updated your
...
NOT update zerobot_common.variable1 to new_value.

Note on imports and circular dependencies:
In general you must avoid loops of A: import B and B: import A
You can not prevent it by re-ordering your lines, python imports the entire file at once.
- the utilities and logfile modules are good examples of safe, self contained imports.
Because this module is used by many, it is the most likely one to show existing circular depencies,
it has to be able to load no matter which modules have been enabled or disabled. 

There are exceptions of non-breaking circular importing with functions:
The site_ops and permissions imports in here have such a loop, we import them, 
they import us. However, they only use references to this module in functions, which
are not being run at the time of importing the file.
So while they are linked, they do not cause circular dependency errors and as 
long as the program ensures that those references have been initialized by the
time those functions are run (which it does) there is no problem with those 
imports when the program runs. I choose this over spending more time to split
them up, it makes sense from an order of operations view.
"""
...
settings_file = "live_settings.json"
...
logfile.log(f"server permissions system loaded and ready.")
...
clan_issue_role_id = settings.get("clan_issue_role_id")
leader_role_id = settings.get("leader_role_id")
...
if sheet_memberlist_enabled:
    ...
    memberlist_sheet_key = settings.get("memberlist_sheet_key")
    memberlist_doc = drive_client.open_by_key(memberlist_sheet_key)
...
def is_staff_member(discord_user):
    ...
    """
    for r in discord_user.roles:
        if r.id == staff_role_id or r.id == clan_issue_role_id or r.id == leader_role_id:
            return True
    return False
...
##
## Common checks for slash commands
##

async def is_staff_check(ctx: discord.Interaction):
    """
    Only returns True if the user has the staff member role.
    """
    # user = the person starting the interaction.
    return is_staff_member(ctx.user)